...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_grlc."):
...
electrum_logger = logging.getLogger("electrum_grlc")
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_grlc."):
        name = name[13:]
...
def configure_logging(config):
    ...
    if is_android:
        ...
        build_config = autoclass("org.electrum_grlc.electrum_grlc.BuildConfig")
    ...
    _logger.info(f"Electrum-GRLC version: {ELECTRUM_VERSION} - https://electrum-grlc.org - {GIT_REPO_URL}")
