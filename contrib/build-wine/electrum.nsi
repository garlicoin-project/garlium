...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-GRLC"
  !define PRODUCT_WEB_SITE "https://github.com/pooler/electrum-grlc"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-grlc-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-grlc\electrum_grlc\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-grlc\*.*"
  File "c:\electrum-grlc\electrum_grlc\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-grlc-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-grlc-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-grlc-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-grlc-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-grlc-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\garlicoin" "" "URL:garlicoin Protocol"
  WriteRegStr HKCU "Software\Classes\garlicoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\garlicoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\garlicoin\shell\open\command" "" "$\"$INSTDIR\electrum-grlc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\garlicoin"
